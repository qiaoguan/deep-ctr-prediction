...
import numpy as np
os.environ['CUDA_VISIBLE_DEVICES'] = '7'  #指定用哪块GPU

'''
A tensorflow implementation of Deep FM
...
flags.DEFINE_string("train_data_dir", "/trainSamples/201812{15,16,17,18,19,20,21}/v1_add_sampling7_tfrecord/", "dir for training data")
flags.DEFINE_string("eval_data_dir", "/testSamples/20181222/v1_tfrecord/", "dir for evaluation data")
...
flags.DEFINE_boolean(name="use_fm", default=False, help="whether use FM")
...
def export_model(model, export_dir, model_column_fn):
  ...
  columns.append(tf.feature_column.numeric_column("user_id", dtype=tf.int64))
  columns.append(tf.feature_column.numeric_column("label", dtype=tf.int64))
  ...
  for epoch in range(FLAGS.num_epochs):
      ...
      for pred_dict in prediction_result:
          ...
          if len(predictions)%(num_samples/10)==0:
              tf.logging.info('predict at step %d/%d', int(float(len(predictions))/num_samples*FLAGS.predict_steps), FLAGS.predict_steps)
